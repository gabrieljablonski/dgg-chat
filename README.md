...
- `DGGChat`: the class that will run the main event loop, invoking the handlers defined in a
`DGGChatHandler` object, as well as allowing you to send whispers and messages in chat.

When a message is received from the websocket, it is redirected to its respective handler 
(and any relevant [special handler](#special-handlers)). The handler class has access to 
...
Aside from [four exceptions](#special-handlers) (and the `on_whisper_sent()`, which receives zero), the custom handler will be a method with one 
argument (besides `self`), the websocket message received. Each handler receives a specific 
type of message. 
...
[`DGGChatHandler.mapping()`](./dgg_chat/handler/_handler.py#L31) property.

Overriding one of the predefined methods will effectively implement a custom handler. The chat 
instance can be accessed via the `self.chat` attribute. A simple example follows:
...
In case you don't use the default method names, the `self.mapping()` property must be overriden. 
Handlers not implemented don't need to be mapped, **but if `self.mapping()` is overriden, you 
must reference all custom methods implemented.**
...
|:----------------------:|:-------------------------:|:--------------------------------------------------------------------------------------------------------------------:|
| `SERVED_CONNECTIONS`   | `on_served_connections()` | Usually the first message received on a new connection. Lists all connected users and a count of served connections. |
| `USER_JOINED`          | `on_user_joined()`        | A user joins the chat.                                                                                               |
| `USER_QUIT`            | `on_user_quit()`          | A user leaves the chat.                                                                                              |
...
| `MUTE`                 | `on_mute()`               | A user got muted.                                                                                                    |
| `UNMUTE`               | `on_unmute()`             | A user got unmuted.                                                                                                  |
| `BAN`                  | `on_ban()`                | A user got banned.                                                                                                   |
| `UNBAN`                | `on_unban()`              | A user got unbanned.                                                                                                 |
| `SUB_ONLY`             | `on_sub_only()`           | Submode got toggled in chat.                                                                                         |
...
| `on_ws_close()`       | Websocket connection got close, usually by calling `DGGChat.disconnect()`.                                                                                              |
...
|:-------------:|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
| `"throttled"` | Messages got sent too fast with `DGGChat.send_chat_message()` and `DGGChat.send_whisper()`. Should almost never happen ([relevant code](./dgg_chat/_dgg_chat.py#L30)). |
| `"duplicate"` | The message sent was the same as the last one.                                                                                                                         |
| `"needlogin"` | Invalid `auth_token` or `session_login` provided. See [authentication](#authentication)                                                                      |
| `"notfound"`  | Target user for `DGGChat.send_whisper()` not found.                                                                                                                    |
...
and whispers can only be sent to users who messaged you first since the connection was established.
...
the more interesting stuff, like sending messages, require your connection to be authenticated.
Be mindful the keys used in these methods expire, so if stuff stops working, check if this might be it.
